...
if secim == "1":
    ...
    if typhe == "1":
        ...
        insert15 = f"INSERT INTO transaction_log(from_account) VALUES({typhe_id})"
    ...
    if typhe == "2":
        ...
        con.commit()
    date = datetime.now()
    insert3 = f"INSERT INTO account_status_history(created_at) VALUES('{date}') WHERE account.id={insert1}"
    cursor.execute(insert3)
    con.commit()
    insert4 = f"INSERT INTO transaction_log(crated_at) VALUES('{date}' WHERE from_account={insert1})"
    cursor.execute(insert4)
    con.commit()
    insert5 = f"INSERT INTO account_balance_history(crated_at) VALUES('{date}' WHERE account_id={insert1})"
    cursor.execute(insert5)
    con.commit()
    insert6 = f"INSERT INTO payments(crated_at) VALUES('{date}' WHERE account_id={insert1})"
...
if secim == "2":
    ...
    cursor.execute(PRO)
    con.commit()
    abh_ab = f"UPDATE account_balance_history SET after_balance={pulum} WHERE id={id_sec}"
    cursor.execute(abh_ab)
    con.commit()
    bf_balance = pulum - yk_m
    abh_bb = f"UPDATE account_balance_history SET before_balance={pulum} WHERE id={id_sec}"
    cursor.execute(abh_bb)
    con.commit()
    today = datetime.now()
    ASH = f"UPDATE account_status_history SET updated_at={today} WHERE account.id={id_sec}"
    cursor.execute(ASH)
    con.commit()
    TL = f"UPDATE transaction_log SET updated_at={today} WHERE from_account={id_sec}"
    cursor.execute(TL)
    con.commit()
    ABH = f"UPDATE account_balance_history SET updated_at={today} WHERE account.id={id_sec}"
    cursor.execute(ABH)
    con.commit()
        ...
        if paasword == pasd:
            ...
            if yukl_azn > ar_cr:
                ...
                if id_yuk is None:
                    ...
                    con.commit()
                    today = datetime.now()
                    ASH = f"UPDATE account_status_history SET updated_at={today} WHERE account_id={hansi_id,yuk_id}"
                    cursor.execute(ASH)
                    con.commit()
                    TL = f"UPDATE transaction_log SET updated_at={today} WHERE from_account={hansi_id,yuk_id} "
                    cursor.execute(TL)
                    con.commit()
                    ABH = f"UPDATE account_balance_history SET updated_at={today} WHERE account_id={hansi_id,yuk_id}"
                    cursor.execute(ABH)
                    con.commit()
                    PAY = f"UPDATE payments SET updated_at={today} WHERE account_id={hansi_id,yuk_id}"
                    cursor.execute(PAY)
                    con.commit()
                    TL_in = f"UPDATE payments SET amount={yukl_azn} WHERE account_id={hansi_id,yuk_id}"
                    cursor.execute(TL_in)
                    con.commit()
                    TL_in = f"UPDATE transaction_log SET amount={yukl_azn} WHERE from_account={hansi_id},to_account{yuk_id}"
                    print("PROSES BAŞA ÇATMIŞIR !!!...")
